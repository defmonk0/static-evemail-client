...
mail.config(function($routeProvider, $locationProvider, ngQuillConfigProvider) {
	...
	var modules = {"toolbar":[["bold","italic","underline"],[{"color":["#ffffff","#b2b2b2","#4c4c4c","#000000","#ffff00","#00ff00","#ff0000","#0000ff","#7f7f00","#007f00","#7f0000","#00007f","#7f007f","#00ffff","#ff00ff","#007fff"]},{"size":["small",false,"large","huge"]}],["link","clean"]]};
	var theme = "bubble";
...
mail.filter("trust", ["$sce", function($sce) {
...
mail.filter("mailHtmlFix", function() {
	return function(value, marketItems) {
		...
		// zKill
		var characters = [1373,1374,1375,1376,1377,1378,1379,1380,1381,1382,1383,1384,1385,1386,34574];
		reg = '<a href="showinfo:(' + characters.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
		...
		var corporations = [2];
		reg = '<a href="showinfo:(' + corporations.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
		...
		var alliances = [16159];
		reg = '<a href="showinfo:(' + alliances.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
		...
		var solarsystems = [5];
		reg = '<a href="showinfo:(' + solarsystems.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
		...
		reg = '<a href="showinfo:(' + constellations.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
		...
		var regions = [3];
		reg = '<a href="showinfo:(' + regions.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
			"<a href=\"http://evemaps.dotlan.net/map/$2\" target=\"_blank\">$3</a>"
		);
		...
		// Fuzzwork
		reg = '<a href="showinfo:(' + marketItems.join("|") + ')\\/\\/(\\d+)">(.*?)<\\/a>';
		value = value.replace(
			new RegExp(reg, "gi"),
...
mail.controller("MailboxController", [
	...
	// ==================== UI FUNCTIONS
		...
		$scope.addRecipient = function(name) {
			...
			var priority = ["character", "corporation", "alliance", "mailing_list"];
			for(p in priority) {
				var type = priority[p];
				...
				for(id in $scope.$store.lookup[type]) {
					if($scope.$store.lookup[type][id] == name) {
						...
						$scope.send.recipients.sort(function(a, b) {
							return $scope.$store.lookup[a.recipient_type][a.recipient_id].localeCompare(
								$scope.$store.lookup[b.recipient_type][b.recipient_id]
							);
						});
			...
			client.Search.get_search(params)
				.then(function(data) {
		...
		$scope.clickMail = function(id) {
			if($scope.open != null && !$scope.mails[$scope.open].is_read) {
				...
				client.Mail.put_characters_character_id_mail_mail_id(params)
					.then(function(data) {
			...
			if($scope.open == id) {
				...
				if($scope.mails[id].full == undefined) {
					...
					client.Mail.get_characters_character_id_mail_mail_id(params)
						.then(function(data) {
		...
		$scope.deleteMail = function(id) {
			if(confirm("Are you sure?")) {
				...
				$scope.loadingMail[id]++;
				client.Mail.delete_characters_character_id_mail_mail_id(params)
					.then(function(data) {
						if($scope.open == id)
							$scope.open = null;
		...
		$scope.loadMore = function() {
			...
			if(last != null)
				params.last_mail_id = last;

			// Actually do the call.
			$scope.loadingMore++;
			client.Mail.get_characters_character_id_mail(params)
				.then(function(data) {
					for(i in data.obj) {
		...
		$scope.parseLookup = function(list, includeType) {
			...
			if(need.length > 0) {
				...
				client.Universe.post_universe_names(params)
					.then(function(data) {
						for(i in data.obj) {
							var item = data.obj[i];
							$scope.$store.lookup[item.category][item.id] = item.name;
						}
						$scope.$apply();
					})
					.catch(function(error) {
		...
		$scope.sendMail = function() {
			...
			if($scope.send.original != null) {
				...
				body += "<br>";
				body += "From: " + $scope.$store.lookup.character[$scope.send.original.full.from];
				body += "<br>";
				body += "Sent: " + $scope.send.original.full.timestamp.slice(0, -1).replace("T", " ");
				body += "<br>";
				body += "To: " + $scope.parseLookup($scope.send.original.recipients, false);
				body += "<br><br>";
			...
			client.Mail.post_characters_character_id_mail(params)
				.then(function(data) {
	...
	// ==================== ONLOAD
		...
		if($scope.hasAuthentication()) {
			...
			var client = new SwaggerClient({
				...
				$scope.loadingMain++;
				client.Mail.get_characters_character_id_mail_lists(params)
					.then(function(data) {
						for(i in data.obj) {
							var item = data.obj[i];
							$scope.$store.lookup['mailing_list'][item.mailing_list_id] = item.name;
						}
						$scope.loadingMain--;
						$scope.$apply();
					})
				...
				$scope.loadingMain++;
				client.Market.get_markets_prices({})
					.then(function(data) {
...
mail.controller("MasterController", [
	"$scope", "$location", "$localStorage", "$sessionStorage",
...
mail.controller("AuthenticationController", [
	...
	// ==================== UI FUNCTIONS
		...
		$scope.oauthRedirect = function() {
			...
			url += "&scope=esi-mail.organize_mail.v1 esi-mail.read_mail.v1 esi-mail.send_mail.v1";
			url += "&state=" + state;
